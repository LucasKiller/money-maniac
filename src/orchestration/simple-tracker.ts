...
export class SimpleFundingProtocol implements FundingProtocol {
  constructor(
    private readonly treasury: TreasuryGateInterface | undefined,
  ...
  async fundChild(childAddress: string, amountCents: number): Promise<{ success: boolean }> {
    ...
    try {
      const outcome = await this.treasury.executeCreditTransfer({
  ...
  async recallCredits(childAddress: string): Promise<{ success: boolean; amountCents: number }> {
    // The parent cannot authorize spending from a child's wallet. Keep the
    // local accounting untouched until the child signs through its own gate.
    void childAddress;
